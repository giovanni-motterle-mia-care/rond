...
func RevokeHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.FailResponseWithCode(w, http.StatusInternalServerError, err.Error(), types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	...
	if len(reqBody.ResourceIDs) == 0 {
		utils.FailResponseWithCode(w, http.StatusBadRequest, "empty resources list", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	if len(reqBody.Subjects) == 0 && len(reqBody.Groups) == 0 {
		utils.FailResponseWithCode(w, http.StatusBadRequest, "empty subjects and groups lists", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	...
	if err != nil {
		...
		utils.FailResponseWithCode(w, http.StatusInternalServerError, "failed find query crud setup", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	...
	if len(bindingsToDelete) > 0 {
		...
		if err != nil {
			...
			utils.FailResponseWithCode(w, http.StatusInternalServerError, "failed delete query crud setup", types.GENERIC_BUSINESS_ERROR_MESSAGE)
			return
		}
		...
		if err := client.Delete(r.Context(), fmt.Sprintf("_q=%s", string(query)), &deleteCrudResponse); err != nil {
			logger.WithField("error", logrus.Fields{"message": err.Error()}).Error("failed crud request")
			utils.FailResponseWithCode(w, http.StatusInternalServerError, "failed crud request for deleting unused bindings", types.GENERIC_BUSINESS_ERROR_MESSAGE)
			return
		}
	...
	if len(bindingsToPatch) > 0 {
		...
		if err := client.PatchBulk(r.Context(), body, &patchCrudResponse); err != nil {
			...
			utils.FailResponseWithCode(
				...
				types.GENERIC_BUSINESS_ERROR_MESSAGE,
			)
			return
		}
...
func GrantHandler(w http.ResponseWriter, r *http.Request) {
	...
	if reqBody.ResourceID == "" {
		utils.FailResponseWithCode(w, http.StatusBadRequest, "missing resource id", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}

	if len(reqBody.Groups) == 0 && len(reqBody.Permissions) == 0 && len(reqBody.Subjects) == 0 && len(reqBody.Roles) == 0 {
		utils.FailResponseWithCode(w, http.StatusBadRequest, "missing body fields, one of groups, permissions, subjects or roles is required", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	...
	if err := client.Post(r.Context(), &bindingToCreate, &bindingIDCreated); err != nil {
		logger.WithField("error", logrus.Fields{"message": err.Error()}).Error("failed crud request")
		utils.FailResponseWithCode(w, http.StatusInternalServerError, "failed crud request for creating bindings", types.GENERIC_BUSINESS_ERROR_MESSAGE)
		return
	}
	logger.WithFields(logrus.Fields{
